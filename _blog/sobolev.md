...
### $L^p$ 空间的 pre-compact set

由Ascoli-Arzela 定理, 我们知道 $C(\overline{\Omega})$ 中的预紧集可以通过一致有界和等度连续来刻画. 那么在 $L^p$ 空间中, 有类似的结论.

$L^p(\Omega)$ 中的集合 $F$ 是预紧集的充分必要条件是
- $F$ 在 $L^p$ 中有界
- 对于 $\forall \epsilon > 0, \exists \delta > 0$ and 一个紧子集 $K \subset \Omega$, 使得对于 $\forall f \in F$, 有
  - $\int_{\Omega \setminus K} |f|^p < \epsilon^p$
  - $\|\tau_h \tilde f - \tilde f\|_{L^p(K)} < \epsilon$, 当 $|h| < \delta$ 时成立.
这里 $\tilde f$ 是 $f$ 的零延拓, $\tau_h \tilde f(x) = \tilde f(x+h)$.

可以猜测到证明思路就是用 mollifier 进行逼近, 然后利用Ascoli-Arzela 定理.

充分性部分, 由于$C_c^\infty(\Omega)$ 在 $L^p(\Omega)$ 中稠密, 因此有对应的$\frac{\epsilon}{3}$ 网. 它们的supp的并就是对应的紧子集 $K$. 平移连续性直接用对应的逼近和这个网中函数的平移连续性来控制.

必要性部分, 不妨取$\Omega$是$R^n$. $\tilde K$ 是 $K$ 中函数的零延拓. 如果$\tilde K$ 在 $L^p(R^n)$ 中有网, 那么把这个网限制到$\Omega$上就是 $K$ 的网. 用 mollifier 进行逼近, 然后在G上利用Ascoli-Arzela 定理, 再把这个网零延拓回去就是 $\tilde K$ 的网.

### 紧嵌入

如果函数可导, 那么它的平移连续性是自动满足的. 因此它总是可以紧嵌入到低阶的Sobolev空间中. 再符合普通的连续嵌入, 得到全部的紧嵌入情形.
